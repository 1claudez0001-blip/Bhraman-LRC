...
export default function BookCard({ book, activeReservation, onReserve }) {
  const disabled = !!activeReservation;
  const btnText = activeReservation ? (activeReservation.status === 'pending' ? 'Pending' : 'Borrowed') : 'Reserve';
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm hover:shadow-md transition flex flex-col">
      <div className="flex items-start justify-between gap-3">
        ...
        <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold bg-green-50 text-ub-green">Available</span>
      </div>
      ...
      <button
        onClick={onReserve}
        disabled={disabled}
        className={`mt-4 w-full py-2.5 rounded-xl text-sm font-semibold transition ${disabled ? 'bg-gray-100 text-ub-gray cursor-not-allowed' : 'bg-ub-red text-white hover:bg-ub-darkRed cursor-pointer'}`}
      >
        {btnText}
      </button>
    </div>
